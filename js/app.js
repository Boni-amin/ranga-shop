...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    div.innerHTML = `<div class="single-product border border-secondary rounded ">
      <div>
      ...
      <h5>Total Review: ${product.rating.count} </h5>
      <h4 class="mt-4">Price: ${product.price}</h4>
      <div class="mt-5">
        <button onclick="addToCart(${product.id},${product.price})" id="addToCart-btn" class="buy-now btn btn-success ">add to cart</button>
        <button onclick="showDetails('${product.id}')" id="details-btn" class="btn btn-success">Details</button>
      </div>
      </div>
      `;
...
const updateTotal = () => {
  ...
  const updateTotalAmount = parseFloat(grandTotal).toFixed(2);
  document.getElementById("total").innerText = updateTotalAmount;
};
loadProducts();
